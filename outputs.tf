...
output "case_configs" {
  ...
  value = {
    for c in var.cases : "${c.case_type}-${c.case_code}-${c.case_date}" => {
      ...
      "ssh_dfiriris"            = format("%s%s", "ssh -i <sshkey> -l <user> -o ProxyCommand='nc -x 127.0.0.1:<proxyport> %h %p' ", module.case["${c.case_type}-${c.case_code}-${c.case_date}"].fqdn_iris)
      ...
      "web_dfiriris"            = format("%s%s", "https://", module.case["${c.case_type}-${c.case_code}-${c.case_date}"].fqdn_iris)
      "web_timesketch"          = format("%s%s", "http://", module.case["${c.case_type}-${c.case_code}-${c.case_date}"].fqdn_worker)
      "web_notebook"            = format("%s%s:%s%s", "http://", module.case["${c.case_type}-${c.case_code}-${c.case_date}"].fqdn_worker, module.case["${c.case_type}-${c.case_code}-${c.case_date}"].jp_port, "/?token=timesketch")
      "userpass_dfiriris_adm"   = format("%s/%s", module.case["${c.case_type}-${c.case_code}-${c.case_date}"].iris_admuser, module.case["${c.case_type}-${c.case_code}-${c.case_date}"].iris_admpass)
